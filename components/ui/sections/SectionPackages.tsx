                ...
                {displayPackages.map((pkg, idx) => (
                  <th
                    ...
                    className={`p-4 text-center text-sm font-medium ${
                      idx === highlightedIndex ? 'rounded-t-2xl bg-slate-100' : ''
                    }`}
                  >
                    {pkg.name}
                  </th>
                ...
                <tr key={fIdx} className="border-b border-black/5">
                  ...
                  {displayPackages.map((_, pIdx) => {
                    ...
                    return (
                      <td
                        key={pIdx}
                        className={`p-4 text-center ${pIdx === highlightedIndex ? 'bg-slate-50' : ''}`}
                      >
                        {hasFeature ? (
                          <RiCheckLine className="mx-auto h-5 w-5 text-emerald-600" aria-label="Tak" />
                        ) : (
                          <RiCloseLine className="mx-auto h-5 w-5 text-slate-300" aria-label="Nie" />
                        )}
                      </td>
                    );
                  })}
            ...
            {displayPackages.map((pkg, pIdx) => (
              <div
                ...
                className={`rounded-2xl border p-4 ${
                  pIdx === highlightedIndex ? 'border-slate-400 bg-slate-50' : 'border-black/10 bg-white'
                }`}
              >
