...
import CalculatorSteps, { type Selections } from './CalculatorSteps';
...
export default function Calculator() {
  ...
  const [activeSteps, setActiveSteps] = useState<Step[]>(baseSteps);
  ...
  const disableNext = Boolean(
    step?.required && !hasSelection && currentStep < activeSteps.length - 1,
  );
  ...
  useEffect(() => {
    ...
    setActiveSteps(prev => {
      const filtered = prev.filter(
        (_, idx) => idx <= currentStep || !prev[idx].metaBranchParent?.startsWith(step.title),
      );
      if (branches.length) {
        filtered.splice(
          currentStep + 1,
  ...
  const calculateEstimate = () => {
    ...
    activeSteps.forEach((st, idx) => {
      (selections[idx] || []).forEach(val => {
        ...
        else if (st.title.toLowerCase().includes('warianty produktów') && opt.multiplier)
