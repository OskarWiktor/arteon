...
import { FONT_OPTIONS } from '@/components/sections/tools/EmailSignatureGenerator/constants';
...
export default function AppearancePanel({ styleConfig, themeId, themePresets, onStyleChange, onApplyTheme, t }: Props) {
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div>
          ...
          <div className="flex items-center gap-2">
            <input type="color" value={styleConfig.accentColor} onChange={(e) => onStyleChange('accentColor', e.target.value)} className="tool-color-picker h-9! w-9!" />
          </div>
        </div>
        <div>
          <p className="tool-label mb-1">{t.appearance.textColor}</p>
          <div className="flex items-center gap-2">
            <input type="color" value={styleConfig.textColor} onChange={(e) => onStyleChange('textColor', e.target.value)} className="tool-color-picker h-9! w-9!" />
          </div>
        </div>
        <div>
          <p className="tool-label mb-1">{t.appearance.backgroundColor}</p>
          <div className="flex items-center gap-2">
            <input type="color" value={styleConfig.backgroundColor} onChange={(e) => onStyleChange('backgroundColor', e.target.value)} className="tool-color-picker h-9! w-9!" />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3">
        <div>
          ...
          <select value={styleConfig.fontFamily} onChange={(e) => onStyleChange('fontFamily', e.target.value)} className="tool-select">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            <PillButton value="small" current={styleConfig.fontSize} label={t.appearance.fontSizeSmall} onChange={(v) => onStyleChange('fontSize', v as FontSizeOption)} />
            <PillButton value="normal" current={styleConfig.fontSize} label={t.appearance.fontSizeNormal} onChange={(v) => onStyleChange('fontSize', v as FontSizeOption)} />
            <PillButton value="large" current={styleConfig.fontSize} label={t.appearance.fontSizeLarge} onChange={(v) => onStyleChange('fontSize', v as FontSizeOption)} />
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="mt-2 flex gap-2">
          <button
            type="button"
            onClick={() => onStyleChange('border', { left: true, right: true, top: true, bottom: true })}
          ...
          <button
            type="button"
            onClick={() => onStyleChange('border', { left: false, right: false, top: false, bottom: false })}
            className={`tool-button ${!styleConfig.border.left && !styleConfig.border.right && !styleConfig.border.top && !styleConfig.border.bottom ? 'tool-button-active' : 'tool-button-inactive'}`}
          >
            {t.border.none}
          </button>
