...
import { FORMAT_LABELS, type ImageToPdfConverterProps, type PdfQueueFile, type ImageFormat } from './types';
...
export default function ImageToPdfConverter({ sourceFormat, acceptMime }: ImageToPdfConverterProps) {
  ...
  return (
    <div className="overflow-hidden">
      <FormatSelector currentSource={sourceFormat} currentTarget="pdf" hasFiles={files.length > 0} />
          ...
          <form onSubmit={handleConvert} className="space-y-4">
            <div>
              ...
              <ToolFileDropzone accept={acceptMime} multiple onFiles={handleAddFiles} className="tool-upload-area">
                <ToolUploadContent
                  dragLabel={t.dragDrop?.replace('{{format}}', sourceLabel) ?? `Drop ${sourceLabel} files here`}
                  clickLabel={t.clickToSelect}
                  formatsLabel={t.supported?.replace('{{format}}', sourceLabel) ?? `Supported: ${sourceLabel}`}
                />
              </ToolFileDropzone>
              {globalError && (
        ...
        <ToolSection aria-label={t.queueAriaLabel} className="space-y-2">
          ...
          {files.length === 0 && (
            <div className="rounded-xl border border-neutral-200 bg-neutral-50 px-4 py-6 text-center">
              <p className="tool-meta">{t.emptyState?.replace('{{format}}', sourceLabel) ?? `Add ${sourceLabel} files to convert to PDF`}</p>
            </div>
          )}
