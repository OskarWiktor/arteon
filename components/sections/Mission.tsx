    ...
    <SectionFour
      ...
      renderItem={({ imageAlt, imageSrc, content, border }, index) => (
        <article role="group" aria-labelledby={`mission-title-${index}`} aria-describedby={`mission-desc-${index}`} tabIndex={0} className="flex flex-col py-2">
          ...
          <h3 id={`mission-title-${index}`} className={`mt-2 mb-4 w-fit text-balance border-b-2 text-2xl font-semibold text-gray-900 capitalize ${border}`} tabIndex={0}>
            {imageAlt}
          </h3>
          <p id={`mission-desc-${index}`} className="text-gray-800 text-balance md:text-lg" tabIndex={0}>
            {content}
          </p>
        </article>
      )}
