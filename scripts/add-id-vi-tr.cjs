...
const toolData = {
  jpgToWebp: {
    ...
    vi: {
      ...
      title: 'Chuyen doi JPG/PNG sang WebP',
      description: 'Giam kich thuoc file hinh anh ma khong mat chat luong. Chuyen doi JPG va PNG sang WebP va tang toc trang web.',
    },
    tr: {
      slug: 'jpg-png-webp-donusturucu',
      title: 'JPG/PNG WebP donusturucu',
      description: 'Kalite kaybetmeden gorsel dosya boyutunu kucultun. JPG ve PNG dosyalarini WebP formatina donusturun ve web sitenizi hizlandirin.',
    },
  },
  imageResize: {
    ...
    vi: {
      ...
      description: 'Chuan bi khung hinh hoan hao cho mang xa hoi hoac trang web cua ban. Chon dinh dang co san hoac nhap kich thuoc tuy chinh.',
    },
  ...
  metaCounter: {
    ...
    vi: {
      ...
      title: 'Kiem tra meta title & description',
      description: 'Kiem tra so ky tu va xem truoc trang cua ban tren Google. Tranh tieu de va mo ta bi cat trong ket qua tim kiem.',
    },
    tr: {
      slug: 'meta-baslik-ve-aciklama-kontrol',
      title: 'Meta baslik ve aciklama kontrolu',
      description: 'Karakter sayisini kontrol edin ve sayfanizin Google onizlemesini gorun. Arama sonuclarinda kesilen baslik ve aciklamalardan kacinin.',
    },
  },
  wordCounter: {
    id: {
    ...
    tr: {
      ...
      title: 'Kelime ve karakter sayaci',
      description: 'Metin uzunlugunu kontrol edin ve ana sayfa, hizmet sayfasi, blog yazisi veya urun aciklamasi icin uygun olup olmadigini degerlendirin.',
    },
  },
  emailSignature: {
    id: {
    ...
    tr: {
      ...
      title: 'E-posta imza olusturucu',
      description: 'Dakikalar icinde profesyonel bir e-posta imzasi olusturun. Hazir HTML kodunu Gmail veya Outlook a kopyalayin.',
    },
  },
  contrastChecker: {
    id: {
    ...
    vi: {
      ...
      title: 'Kiem tra do tuong phan mau',
      description: 'Kiem tra xem mau chu va mau nen cua ban co de doc khong. Cong cu tinh do tuong phan theo WCAG va giup chon mau phu hop.',
    },
    tr: {
      slug: 'renk-kontrast-kontrolu',
      title: 'Renk kontrast kontrolu',
      description: 'Metin ve arka plan renklerinizin okunakli olup olmadigini kontrol edin. Arac WCAG a gore kontrasti hesaplar ve dogru rengi secmenize yardimci olur.',
    },
  },
  paletteExtractor: {
    id: {
    ...
    vi: {
      ...
      title: 'Trich xuat mau tu hinh anh',
      description: 'Tai len anh hoac logo va cong cu se trich xuat cac mau chu dao. Sao chep ma mau duoc tao voi mot lan nhan.',
    },
    tr: {
      slug: 'gorsel-renk-cikarici',
      title: 'Gorsel renk cikarici',
      description: 'Bir fotograf veya logo yukleyin ve arac baskun renkleri cikaracaktir. Olusturulan renk kodlarini tek tikla kopyalayin.',
    },
  },
  colorPalette: {
  ...
  qrCode: {
    ...
    tr: {
      ...
      title: 'Ucretsiz QR kod olusturucu',
      description: 'Web sitesi, vCard, menu veya brosur icin QR kod olusturun. PNG ve SVG olarak disari aktarin, giris veya sinir yok.',
    },
  },
};
...
for (const [sectionKey, titles] of Object.entries(sectionTitles)) {
  ...
  const sectionRegex = new RegExp(`(key: '${sectionKey}',[\\s\\S]*?hu: \\{ title: '[^']+' \\},\\n)(\\s+\\},)`);
...
for (const [toolKey, locales] of Object.entries(toolData)) {
  const toolRegex = new RegExp(`(key: '${toolKey}',[\\s\\S]*?hu: \\{[\\s\\S]*?\\},\\n)(\\s+\\},\\n\\s+\\},)`);
...
if (!localesSrc.includes("import id from '@/dictionaries/id.json'")) {
  ...
  // Add to dicts object
  localesSrc = localesSrc.replace(/hu,\n\} as const;/, 'hu,\n  id,\n  vi,\n  tr,\n} as const;');
