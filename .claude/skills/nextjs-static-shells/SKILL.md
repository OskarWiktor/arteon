...
| Field      | Value                                     |
| ---------- | ----------------------------------------- |
| Identifier | `joelhooks-joelclaw-nextjs-static-shells` |
| Version    | 1.0.0                                     |
| Author     | joelhooks                                 |
...
**GitHub:** [joelhooks/joelclaw](https://github.com/joelhooks/joelclaw) — ⭐ 43 | Forks: 3

---

## Skill Overview

nextjs-static-shells documents a compact set of Next.js 16 architecture patterns for maximizing static rendering while supporting targeted dynamic personalization. It prescribes cached shells with dynamic slots, explicit 'use cache' boundaries, provider islands (scoped client providers), and a deliberate link preloading/prefetch strategy. Use it when building or refactoring routes to keep most markup static, split entry vs static renderers to minimize client JS, implement 'use cache' for dynamic content fragments, or when routes unexpectedly become dynamic. Key features: how to declare cache boundaries, design dynamic slots for personalization, isolate client state in provider islands, and tune Next.js link prefetching. Core advantages include reduced SSR load, smaller client bundles, faster TTFB and navigations, predictable caching behavior, and safer progressive personalization in hybrid static/dynamic apps.
...
```bash
npx -y @lobehub/market-cli skills install joelhooks-joelclaw-nextjs-static-shells
```
...
npx -y @lobehub/market-cli skills install joelhooks-joelclaw-nextjs-static-shells --agent open-claw

# Claude Code (installs to ./.claude/skills/)
npx -y @lobehub/market-cli skills install joelhooks-joelclaw-nextjs-static-shells --agent claude-code

# Codex (installs to ./.agents/skills/)
npx -y @lobehub/market-cli skills install joelhooks-joelclaw-nextjs-static-shells --agent codex

# Cursor (installs to ./.cursor/skills/)
npx -y @lobehub/market-cli skills install joelhooks-joelclaw-nextjs-static-shells --agent cursor
```

Install a specific version:

```bash
npx -y @lobehub/market-cli skills install joelhooks-joelclaw-nextjs-static-shells --version 1.0.0
```
...
```bash
npx -y @lobehub/market-cli skills rate joelhooks-joelclaw-nextjs-static-shells --score <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comment joelhooks-joelclaw-nextjs-static-shells -c "Your feedback here" --rating <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills uncomment joelhooks-joelclaw-nextjs-static-shells
```
...
```bash
npx -y @lobehub/market-cli skills comments joelhooks-joelclaw-nextjs-static-shells
```
